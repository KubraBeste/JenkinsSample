pipeline {
    ...
    stages {
        stage('Build') {
            steps {
                echo 'Build Starts!'
                bat "\"C:/Program Files/dotnet/dotnet.exe\" restore \"${workspace}/JenkinsSampleProject.sln\""
                bat "\"C:/Program Files/dotnet/dotnet.exe\" build \"${workspace}/JenkinsSampleProject.sln\""
                echo 'Build Ends'
            }
        }
